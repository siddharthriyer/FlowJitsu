...
def _assign_IDS_to_datafiles(datafiles, parser, measurement_class=None, **kwargs):
    ...
    Dict of ID:datafile
    """
    if isinstance(parser, collections.abc.Mapping):
...
class MeasurementCollection(collections.abc.MutableMapping, BaseObject):
    """
    ...
    def __init__(self, ID, measurements):
        ...
        self.ID = ID
        self.data = {}
        if isinstance(measurements, collections.abc.Mapping):
    ...
    def filter(self, criteria, applyto='measurement', ID=None):
        ...
        fil = criteria
        new = self.copy()
        if isinstance(applyto, collections.abc.Mapping):
...
class OrderedCollection(MeasurementCollection):
    ...
    def _get_ID2position_mapper(self, position_mapper):
        ...
        elif isinstance(position_mapper, collections.abc.Mapping):
